...
class Kodoutputinput extends Kawal 
{
#----------------------------------------------------------------------------------------------------------------------------#
	...
	//private function semakKawalProses($sv, $kawalID, $prosesID, $cariKawal = NULL, $cariProses = NULL)
	private function semakKawalProses($sv, $kawalID, $prosesID, $bawaPencam)
	{
		...
		if (isset($this->papar->prosesID) && $this->papar->prosesID != null):
			if($sv=='101')
			{
				...
				$this->papar->perangkaan['gaji']     = $this->papar->prosesID[$kp.'08_2010'][0]['F2180'];
				$this->papar->perangkaan['susut']    = $this->papar->prosesID[$kp.'08_2010'][0]['F2161'];
				$this->papar->perangkaan['aset']     = $this->papar->prosesID[$kp.'04_2010'][0]['F0888'];
				$this->papar->perangkaan['asetsewa'] = $this->papar->prosesID[$kp.'04_2010'][0]['F0988'];

				# bentuk soalan 4 - aset
				$this->semak_aset($asetIndustri,$aset,$prosesID, $sv);
...
//*/
		...
		{	
			foreach (dpt_senarai('kawalan_tahunan') as $key => $myTable) 
				$kawalID[$myTable] = $this->tanya->//paparSQL 
					cariData 
					($semak = 'K', $myTable, $sv, $cariID);

			foreach ($this->senarai_jadual($sv) as $key => $myTable)
				$prosesID[$myTable] = $this->tanya->//paparSQL 
					cariData
					($semak = 'P', $myTable, $sv, $cariID, $mula, $akhir);

			# paparkan data kesID yang ada nilai sahaja
			$this->semakKawalProses($sv, $kawalID, $prosesID, $bawaPencam);
			//$this->semakKawalProses($sv, $kawalID, $prosesID, $cariKawal, $cariProses);
			...
			$this->papar->sv = $sv;
			$this->papar->thn_mula = $mula;
			$this->papar->thn_akhir = $akhir;
		} 
		else 
			$this->papar->carian='[id:0]'; 

			/*echo '<pre>'; # semak pembolehubah
			echo '$this->papar->keterangan='; print_r($this->papar->keterangan); 
			echo '<hr>$this->papar->kawalID='; print_r($this->papar->kawalID);
			echo '<hr>$this->papar->prosesID='; print_r($this->papar->prosesID);
			echo '<hr>$this->papar->kod_produk='; print_r($this->papar->kod_produk); # khas untuk survey 205
			echo '<hr>$this->papar->perangkaan='; print_r($this->papar->perangkaan); 
	...
	private function semak_produk($cari) # khas untuk survey 205
	{
		...
		{# mula ulang table
			...
			elseif ($myTable == 's14') 
			{
				...
				$medan = '*';
				$this->papar->kod_produk[$myTable] = 
					$this->tanya->cariSemuaData($myTable, $medan, $cari, NULL);
			}
			else
			{
				$this->papar->kod_produk[$myTable] = 
	...
	{
		$senarai = SemakKP::cariKeterangan($kesID);
		...
		# semak data
		//echo '<pre>$this->papar->keterangan='; print_r($this->papar->keterangan); echo '</pre>';
	}
	...
	private function semak_aset($asetIndustri, $aset, $paparID, $sv) 
	{# khas untuk soalan aset
		...
		foreach ($asetIndustri as $key => $myTable):
			if ($aset==$myTable && in_array($aset,$asetIndustri) )
			{
				...
				@$this->papar->kod_produk['harta_' . $myTable] = ($sv=='101') ?
					SemakKP::binaAset($paparID[$myTable][0]) : Borang::binaAset($paparID[$myTable][0]);
				@$this->papar->kod_produk['jadualHarta'] = ($sv=='101') ?
					SemakKP::binaAset($paparID[$myTable][0]) : Borang::inputAset($paparID[$myTable][0]);
			}
			elseif ($aset==null)
			{
	...
	private function semak_staf($jadualStaf, $prosesID, $kp = '000')
